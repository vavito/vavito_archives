...
describe('AdminCampaignsController', () => {
  ...
  it('delega criação, preview, edição, exclusão e envio ao service', async () => {
    ...
    const createDto = {
      postIds: ['bbd448e1-20bf-4fb3-aa26-f1ff9e7194f6'],
      subject: 'Novo artigo',
    };
    ...
    await controller.delete(user, campaignId);
    ...
    expect(deleteCampaign).toHaveBeenCalledWith(user.id, campaignId);
